...
from prune.search import search_mac, search_latency, search_random
...
parser.add_argument("--rnd_constraint_heads", type=float, default=None, help="Percentage of heads to keep for random pruning")
parser.add_argument("--rnd_constraint_neurons", type=float, default=None, help="Percentage of neurons to keep for random pruning")
...
def main():
    ...
    if args.output_dir is None:
        args.output_dir = os.path.join(
            ...
            args.task_name,
            args.metric,
            str(args.constraint) if args.metric != "random" else str(args.rnd_constraint_heads) + "_" + str(args.rnd_constraint_neurons),
            f"seed_{args.seed}",
        )
    ...
    if args.metric == "mac":
        ...
        teacher_head_constraint = get_pruning_schedule(target=args.rnd_constraint_heads, num_iter=2)[0]
        teacher_neurons_constraint = get_pruning_schedule(target=args.rnd_constraint_neurons, num_iter=2)[0]
        ...
        head_mask, neuron_mask = search_random(
            config,
            args.rnd_constraint_heads,
            args.rnd_constraint_neurons
        )
        logger.info(f"Pruned heads randomly to: {args.rnd_constraint_heads * 100.0:.2f} %")
        logger.info(f"Pruned neurons randomly to: {args.rnd_constraint_neurons * 100.0:.2f} %")
    else:
        return
