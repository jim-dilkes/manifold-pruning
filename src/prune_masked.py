...
from pruning.masked.dataset.glue import glue_dataset, max_seq_length, avg_seq_length
from pruning.masked.dataset.squad import squad_dataset
from pruning.masked.efficiency.mac import compute_mask_mac
from pruning.masked.efficiency.latency import estimate_latency
from pruning.masked.prune.fisher import collect_mask_grads
from pruning.masked.prune.search import search_mac, search_latency, search_random
from pruning.masked.prune.rearrange import rearrange_mask
from pruning.masked.prune.rescale import rescale_mask
from pruning.masked.evaluate.nlp import test_accuracy
from pruning.masked.utils.schedule import get_pruning_schedule
...
parser.add_argument("--model_name", type=str, default="bert-base-uncased-squad2")
parser.add_argument("--task_name", type=str, default="squad_v2", choices=[
    "mnli",
    "qqp",
    "qnli",
    "sst2",
...
parser.add_argument("--ckpt_dir", type=str, default="models/bert-base-uncased-squad2")
...
parser.add_argument("--constraint_heads", type=float, default=None, help="Percentage of heads to keep for random pruning")
parser.add_argument("--constraint_neurons", type=float, default=None, help="Percentage of neurons to keep for random pruning")
...
def main():
    ...
    if args.output_dir is None:
        args.output_dir = os.path.join(
            ...
            args.task_name,
            args.metric,
            str(args.constraint) if args.metric != "random" else str(args.constraint_heads) + "_" + str(args.constraint_neurons),
            f"seed_{args.seed}",
        )
    ...
    # full_neuron_mask = torch.ones(config.num_hidden_layers, config.hidden_dim).cuda() # distilbert
    full_neuron_mask = torch.ones(config.num_hidden_layers, config.intermediate_size).cuda() # bert

    start = time.time()
    # Search the optimal mask
    logger.info("Collecting mask gradients...")
    ...
    if args.metric == "mac":
        ...
        teacher_head_constraint = get_pruning_schedule(target=args.constraint_heads, num_iter=2)[0]
        teacher_neurons_constraint = get_pruning_schedule(target=args.constraint_neurons, num_iter=2)[0]
        teacher_head_mask, teacher_neuron_mask = search_random(
            config,
            teacher_head_constraint,
            teacher_neurons_constraint
        )
        head_mask, neuron_mask = search_random(
            config,
            args.constraint_heads,
            args.constraint_neurons
        )
        logger.info(f"Pruned heads randomly to: {args.constraint_heads * 100.0:.2f} %")
        logger.info(f"Pruned neurons randomly to: {args.constraint_neurons * 100.0:.2f} %")
    else:
        return
